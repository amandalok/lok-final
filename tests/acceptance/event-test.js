...
test('display event', async function(assert) {
server.create('post', {
  name: 'View event',
  date: 'View event',
  location: 'View event',
...
assert.dom('[data-test="date"]').exists({ count:2});
 assert.dom('[data-test="location"]').exists({ count:2 });
  assert.dom('[data-test="contact"]').exists({ count:2});
